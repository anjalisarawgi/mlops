...
def train(config) -> None:
    ...
    hparams = config.training
    # Load model config
    model_cfg = OmegaConf.load("../../../mlops_project/config/model_conf.yaml")
    hparams_models = model_cfg.model
    wandb.init(project="mlops_project")
    model = MyAwesomeModel(hparams_models).to(DEVICE)
    ...
    statistics = {"train_loss": [], "train_accuracy": [], "average_loss": []}
    for epoch in range(10):
        ...
        statistics["average_loss"].append(average_loss)
        log.info(f"Epoch {epoch}, average loss: {average_loss:.4f}")
        wandb.log({"average_loss": average_loss, "epoch": epoch})
    ...
    axs[2].plot(statistics["average_loss"])
