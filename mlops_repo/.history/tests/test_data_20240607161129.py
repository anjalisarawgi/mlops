...
def test_data():
    ...
    assert len(
        dataset.train_images) == 60000, "Dataset did not have the correct number of training samples"
    assert len(
        dataset.test_images) == 10000, "Dataset did not have the correct number of test samples"
    assert all(img.shape == (28, 28)
               for img in dataset.train_images), "Training images did not have the correct shape"
    assert all(img.shape == (28, 28)
