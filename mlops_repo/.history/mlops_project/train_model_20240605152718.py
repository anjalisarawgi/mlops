...
def train(config) -> None:
    ...
    weight_snapshots = []
    # Function to plot PCA of weights

    def plot_pca(weights, epoch):
        pca = PCA(n_components=2)
        weights_pca = pca.fit_transform(weights)
        plt.figure(figsize=(8, 6))
        plt.scatter(weights_pca[:, 0], weights_pca[:, 1], c='blue', marker='o')
        plt.title(f'PCA of Weights at Epoch {epoch}')
        plt.xlabel('Principal Component 1')
        plt.ylabel('Principal Component 2')
        plt.savefig(f'pca_weights_epoch_{epoch}.png')
        plt.show()
            ...
            if i % 100 == 0:
                ...
                if i == 0:
                    ...
                    wandb.log({"examples": [wandb.Image(im)
                              for im in images_t]})
            # Capture the weights of the model
            weight_snapshots.append(
                model.fc1.weight.data.cpu().numpy().flatten())
            if epoch % 10 == 0:
                plot_pca(np.array(weight_snapshots), epoch)
