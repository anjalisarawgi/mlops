...
# # def evaluate_continuity(original_data, reduced_data, n_neighbors=3):
...
def train(config) -> None:
    ...
    model = MyAwesomeModel(hparams_models).to(DEVICE)
    train_img_path = to_absolute_path("data/processed/train_images.pt")
    train_target_path = to_absolute_path("data/processed/train_targets.pt")

    if not os.path.exists(train_img_path) or not os.path.exists(train_target_path):
