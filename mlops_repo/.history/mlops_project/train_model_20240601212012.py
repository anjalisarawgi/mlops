    ...
    for epoch in range(hparams["epochs"]):
        ...
        for i, (img, target) in enumerate(train_dataloader):
            ...
            if i % 100 == 0:
                ...
                # Log images
                wandb.log({"input_images": [wandb.Image(img[i]) for i in range(len(img))],
                           "predicted_labels": [wandb.Image(y_pred[i]) for i in range(len(y_pred))],
                           "true_labels": [wandb.Image(target[i]) for i in range(len(target))]})
                # Alternatively, you can log a single image using:
                # wandb.log({"input_image": wandb.Image(img[0]),
                #            "predicted_label": wandb.Image(y_pred[0]),
                #            "true_label": wandb.Image(target[0])})
